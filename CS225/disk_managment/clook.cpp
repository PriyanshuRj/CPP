...
int main()
{
    ...
    cin >> head;
    cout << "Enter string of disk location : ";
    cin.ignore();
...
direction:
    ...
    {
        if (v.back() > head)
            cout << "Total Head Movement: " << head + 2 * v.back() - 2 * v.front() - more << endl;
        else
    ...
    {
        if (v.front() < head)
            cout << "Total Head Movement: " << 2 * v.back() + less - head - 2 * v.front() << endl;
        else
